...
export function createSolvedBoard(): Tile[] {
...
export function getMovableTiles(emptyIndex: number): number[] {
  ...
  if (row > 0) neighbours.push(emptyIndex - BOARD_SIZE); // arriba
  if (row < BOARD_SIZE - 1) neighbours.push(emptyIndex + BOARD_SIZE); // abajo
  if (col > 0) neighbours.push(emptyIndex - 1); // izquierda
  if (col < BOARD_SIZE - 1) neighbours.push(emptyIndex + 1); // derecha

  return neighbours;
}

/** Mueve la ficha en `tileIndex` hacia la celda vacía (si es válido) */
export function moveTile(tiles: Tile[], tileIndex: number): Tile[] | null {
