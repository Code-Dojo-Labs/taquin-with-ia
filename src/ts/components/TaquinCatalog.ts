...
export class TaquinCatalog extends HTMLElement {
  ...
  private handleSelect(tiles: Tile[]): void {
    this.dispatchEvent(
      new CustomEvent<{ tiles: Tile[] }>('puzzle-selected', {
        detail: { tiles },
        bubbles: true,
        composed: true,
      })
  ...
  private renderThumbnail(tiles: Tile[], idx: number): string {
    const cells = tiles
      .map((t) =>
        t.value === 0
  ...
  render(): void {
    ...
    this.shadow.getElementById('overlay')?.addEventListener('click', (e) => {
      if ((e.target as HTMLElement).id === 'overlay') this.handleClose();
    });
    ...
    this.shadow.querySelectorAll<HTMLElement>('.thumb').forEach((el, i) => {
