...
const boardEl = document.querySelector('taquin-board');
const hudEl = document.querySelector('taquin-hud');
const modalEl = document.querySelector('taquin-modal');
const catalogEl = document.querySelector('taquin-catalog');
...
function renderBoard() {
    ...
    state.tiles.forEach((tile, index) => {
        const el = document.createElement('taquin-tile');
        if (tile.value === 0) {
...
function handleTileClick(index) {
    ...
    if (!getMovableTiles(emptyIndex).includes(index)) {
        ...
        const tileEl = boardEl.querySelectorAll('taquin-tile')[index];
